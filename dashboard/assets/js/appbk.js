(function ($) {
    ...
    ensureGoogleMaps()
      .then(function () {
        ...
        (features || []).forEach(function (feature) {
          ...
          addHeatCircle(map, position, props, mode);
          const marker = new google.maps.Marker({
            map,
            position,
            title: props.nombre || props.codigo,
            icon: {
              ...
              strokeOpacity: 0.75,
              strokeWeight: 1,
              scale: props.capa === "candidatos" ? 8 : 6,
            },
          });
          marker.addListener("click", function () {
            info.setContent(`
              <div style="color:#111;min-width:180px">
                <strong>${escapeHtml(props.nombre || props.codigo)}</strong><br>
  ...
  function renderLegend(selector, items) {
    $(selector).html(items.map(function (item) {
      return `<span class="legend-item"><i class="legend-dot" style="background:${item[1]}"></i>${escapeHtml(item[0])}</span>`;
    }).join(""));
  }
